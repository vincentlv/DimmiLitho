...
class Source:
    ...
    def update(self):     
        ...
        if self.type == 'conventional':
            ...
            s = Edeta(self.smooth_deta, self.sigma_out - self.r)*\
                Edeta(self.smooth_deta, self.r - self.sigma_in)*self.s0
            self.data = s
        elif self.type == 'quasar':  
            s = Edeta(self.smooth_deta,self.sigma_out-self.r)*Edeta(self.smooth_deta,self.r-self.sigma_in)*\
               (Edeta(self.smooth_deta,self.openAngle-np.abs(1.0*math.pi-np.abs(self.shiftAngle-self.theta))) +\
                Edeta(self.smooth_deta,self.openAngle-np.abs(0.5*math.pi-np.abs(self.shiftAngle-self.theta))) +\
                Edeta(self.smooth_deta,self.openAngle-np.abs(-0.5*math.pi-np.abs(self.shiftAngle-self.theta)))+\
                Edeta(self.smooth_deta,self.openAngle-np.abs(-0.0*math.pi-np.abs(self.shiftAngle-self.theta))))*self.s0
            self.data = s
        elif self.type == 'dipole': 
            s = Edeta(self.smooth_deta,self.sigma_out-self.r)*Edeta(self.smooth_deta,self.r-self.sigma_in)*\
                (Edeta(self.smooth_deta,self.openAngle-np.abs(self.shiftAngle-self.theta)) +\
                Edeta(self.smooth_deta,self.openAngle-np.abs(math.pi-np.abs(self.shiftAngle-self.theta))))*self.s0;
            self.data = s
        else:
            s = Edeta(self.smooth_deta, self.sigma_out - self.r)*\
                Edeta(self.smooth_deta, self.r - self.sigma_in)*self.s0
    ...
    def ifft(self):               
        ...
        self.spatMutualData=np.fft.fftshift(np.fft.ifft2(np.fft.ifftshift(self.mdata)))*normlize
